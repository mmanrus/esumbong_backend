...
export const createConcern = async (data, categoryId, userId) => {
  ...
  const officials = await prisma.user.findMany({
    where: {
      ...
      barangayId: user.barangayId,   // ← scoped
      isActive: true,
    },
    select: {
      id: true,
  ...
  await Promise.all(
    officials.map(async (official) => {
      ...
      sendToUser(official.id, {
        ...
        notification: { url, message, itemId: newConcern.id, type: "concern", barangayId }
      })
...
export const getConcernById = async (concernId, barangayId) => {
...
export const getAllConcerns = async ({ search, status, archived, validation, recent, spam, isAnonymous, cursor, take = 20, barangayId }) => {
...
export const getConcernStats = async (userId, isOfficial, barangayId) => {
  const result = await prisma.concern.groupBy({
    by: ["status"],
    where: isOfficial ? { barangayId } : { userId },
    _count: {
      status: true
    }
  })
...
export const getResidentConcerns = async (userId, cursor, take = 20, barangayId) => {
  const user = await prisma.user.findFirst({
    where: {
      id: userId, barangayId
    }
  })
  if (!user) {

  ...
  const results = await prisma.user.findMany({
    ...
    where: {
      AND: [
        ...
        { type: { not: "superAdmin" } },
        // Scope to admin's barangay if provided
    ...
    orderBy: {
      createAt: "desc" // typo use as createAt not createdAt
    },
    take: take + 1,
  });
 ...
 * The refresh token is a 7-day JWT signed with REFRESH_SECRET.
 */
export const refreshAccessToken = async (refreshToken) => {
  try {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, type: true, isVerified: true, isActive: true, isLocked: true },
    });

    if (!user) throw new AppError("User not found.", 404);
    if (!user.isActive) throw new AppError("Account is restricted.", 403);
    if (user.isLocked) throw new AppError("Account is locked.", 423);
    ...
    // jwt.verify throws JsonWebTokenError / TokenExpiredError — treat both as 401
...
export const sendVerify = async (userId, metaData) => {
  ...
  const message = `${user.fullname} has requested for verification.`;
  ...
  await Promise.all(
    admins.map(async (admin) => {
      ...
      sendToUser(admin.id, {
        ...
        notification: { message, itemId: userId, type: "verification" },
      });
    })
  );

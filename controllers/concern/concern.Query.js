...
export const getConcernById = async (req, res) => {
  ...
  try {
    const concern = await concernService.getConcernById(concernId, barangayId);
    if (!concern) {
      return res.status(404).json({ error: "Concern not found" });
    }
...
export const getConcernStats = async (req, res) => {
  ...
  try {
    const stats = await concernService.getConcernStats(Number(userId), isOfficial, barangayId)
...
export const getConcernHistory = async (req, res) => {
  ...
  try {
    const results = await concernService.getUpdatedConcerns(userId, cursor ? parseInt(cursor) : undefined, barangayId)

...
export const createConcern = async (req, res) => {
  const {
    title, details, needsBarangayAssistance,
    categoryId, other, location, media, isSpam, isAnonymous,
  } = req.body;

  // Fix: string "false" is truthy — always compare explicitly
  const spam = isSpam === true || isSpam === "true";
  const barangayId = req.user.barangayId
  if (!barangayId) {
    return res.status(400).json({
      error: "Resident not assigned to a barangay"
    })
  }
    ...
    await concernService.createConcern(
      {
        title,
        details,
        needsBarangayAssistance: needsBarangayAssistance === true || needsBarangayAssistance === "true",
        location,
        isSpam: spam,

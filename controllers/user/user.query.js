...
export const getAllUsers = async (req, res) => {
  const { search, type, cursor } = req.query
  // Admins are scoped to their barangay. superAdmin sees all (barangayId = null).
  const barangayId = req.user.type === "superAdmin" ? null : req.user.barangayId;
  try {
    const users = await userService.getAllUsers({ search, type, barangayId }, cursor? parseInt(cursor) : undefined);

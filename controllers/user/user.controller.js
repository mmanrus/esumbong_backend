...
export const createUser = async (req, res) => {
  const { email, password, fullname, age, type, address, contactNumber, barangayId } = req.body;

  if (!email || !password || !fullname || !address || !type || !contactNumber || !barangayId) {
    return res.status(400).json({
      error: "Missing required fields: email, password, fullname, address, type, contactNumber, barangayId",
    });
  }

  try {
    const newUser = await userService.createUser({
      email, password, fullname, age, type, address, contactNumber,
...
export const refreshToken = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(result); // { access: "eyJ..." }
  } catch (error) {
    if (process.env.NODE_ENV === "development") {
